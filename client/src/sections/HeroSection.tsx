...
export default function HeroSection() {
    ...
    return (
        <div className="relative flex flex-col items-center justify-center px-4 md:px-16 lg:px-24 xl:px-32">
            ...
            <motion.a href="https://prebuiltui.com?utm_source=pixels" className="group flex items-center gap-2 rounded-full p-1 pr-3 mt-44 text-yellow-100 bg-yellow-200/15"
                initial={{ y: -20, opacity: 0 }}
                whileInView={{ y: 0, opacity: 1 }}
                viewport={{ once: true }}
                transition={{ delay: 0.2, type: "spring", stiffness: 320, damping: 70, mass: 1 }}
            >
                <span className="bg-yellow-800 text-white text-xs px-3.5 py-1 rounded-full">
                    NEW
                </span>
            ...
            >
                <button onClick={() => navigate('/generate')} className="bg-yellow-600 hover:bg-yellow-700 text-white rounded-full px-7 h-11">
                    Generate Now
                </button>
                <button className="flex items-center gap-2 border border-yellow-900 hover:bg-yellow-950/50 transition rounded-full px-6 h-11">
