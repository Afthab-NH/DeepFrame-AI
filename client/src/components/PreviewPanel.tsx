            ...
            {isLoading && (
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-black/25">
                    ...
                    <div className="text-center">
                        <p className="text-sm font-medium text-zinc-400">Generating your Thumbnail via AI</p>
                        <p className="mt-1 text-xs text-zinc-400">This may take upto 20-30 seconds</p>
                    </div>
                </div>
            )}
            ...
            {!isLoading && thumbnail?.image_url && (
                <div className="group relative h-full w-full">
                    ...
                    <div className="absolute inset-0 flex items-end justify-center bg-black/10 opacity-0 transition-opacity group-hover:opacity-100">
                        <button type="button"
                         className="mb-6 flex items-center gap-2 rounded-md px-5 py-2.5 text-xs font-medium transition bg-white/30 ring-2 ring-white/40
                         backdrop-blur hover:scale-105 active:scale-95"
                         onClick={onDownload} >
